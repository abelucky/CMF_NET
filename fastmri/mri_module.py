...
from fastmri.data.mri_data import IXIdataset
...
class MriModule(pl.LightningModule):
    ...
    def train_dataloader(self):
        return self._create_data_loader(self.train_data_transform(),
    ...
    def test_dataloader(self):
        return self._create_data_loader(self.test_data_transform(),
                                        data_partition=self.test_split, sample_rate=1.0,
        )
    ...
    #     visualize_size_targets = val_targets[0].shape
    #     val_outputs = [x[0] for x in val_outputs if x.shape == visualize_size]
    #     val_targets = [x[0] for x in val_targets if x.shape == visualize_size_targets]
    ...
    def validation_epoch_end(self, val_logs):
        ...
        # )
        # fastmri.save_reconstructions(
        #     outputs, self.exp_dir / self.exp_name / "outputs"
        # )
        # fastmri.save_reconstructions(
        #     targets, self.exp_dir / self.exp_name / "targets"
        # )

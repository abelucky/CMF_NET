...
from fastmri.data.transforms import DataTransform as DataTransform
from fastmri.data.subsample import create_mask_for_mask_type
from fastmri.models.network import net
...
class SRModule(MriModule):
    ...
    def forward(self, targetpd, image):
        tar1, tar2 = self.Net(targetpd.unsqueeze(1), image.unsqueeze(1))
        #tar1, tar2 = self.Net(targetpd, image)
        tar1 = tar1.squeeze(1)
        tar2 = tar2.squeeze(1)
        return tar1, tar2

    def training_step(self, batch, batch_idx):
        image, target, mean, std, fname, slice_num, lr_ori = batch[1]#pdfs
        imagepd, targetpd, meanpd, stdpd, fnamepd, slice_numpd, ori = batch[0]#pd

        tar1, tar2 = self(ori, image)

        loss_tar1 = F.l1_loss(tar1, lr_ori.squeeze(1))
        loss_tar2 = F.l1_loss(tar2, target.squeeze(1))
        loss = loss_tar1+loss_tar2
